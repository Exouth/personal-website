...
export const useAnimation = () => {
  ...
  const staggerContainer = useCallback(
    (delayChildren: number = 0.1, initialDelay: number = 0): AnimationProps => ({
      initial: { opacity: 0 },
      animate: { opacity: 1 },
      transition: {
  ...
  const hoverScale = useCallback(
    (scale: number = 1.05): AnimationProps => ({
      whileHover: {
        scale,
        y: -5,
      },
   ...
   * @param scale Scaling factor (optional)
   */
  const hoverScaleOnly = useCallback(
    (scale: number = 1.05): AnimationProps => ({
      whileHover: {
        scale,
      },
   ...
   */
  const hoverUp = useCallback(
    (distance: number = 5): AnimationProps => ({
      whileHover: {
        y: -distance,
      },
      whileTap: { y: -1 },
